...
function saveTransaction(data) {
  var sheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Kirim Chiqim");
  var batchId =
    data.formMode === "add" && data.rowId
      ? data.rowId
  ...
  if (data.formMode === "add") {
    ...
    if (data.type === "Kirim") {
      ...
      var originalUSD =
        data.payment === "Dollar" && data.originalUSD ? data.originalUSD : null;
      rowsToAdd.push([
        dateStr,
        firmVal,
        data.point,
      ...
      var finalChiqim =
        source === "Dollar" && usdAmount > 0 ? usdAmount : mainAmount;
      var finalKirim =
        dest === "Dollar" && usdAmount > 0 ? usdAmount : mainAmount;
    ...
    if (rowsToAdd.length > 0) {
      ...
      sheet
        .getRange(lastRow + 1, 1, rowsToAdd.length, numCols)
        .setValues(rowsToAdd);
    }
  } else {
    var lastRow = sheet.getLastRow();
    ...
    if (data.type === "Kirim") {
      ...
      for (var i = 0; i < idValues.length; i++) {
        if (idValues[i][0] == targetId) rowsToDelete.push(i + 2);
      }
      ...
      if (rowsToAdd.length > 0)
        sheet
          .getRange(sheet.getLastRow() + 1, 1, rowsToAdd.length, 9)
          .setValues(rowsToAdd);
    } else if (data.type === "Chiqim") {
      for (var i = 0; i < idValues.length; i++) {
      ...
      if (rowIndex > 0) {
        ...
        sheet
          .getRange(rowIndex, 4, 1, 5)
          .setValues([
            [data.category, data.payment, 0, data.amount, data.note],
          ]);
      }
    } else if (data.type === "Otkazma") {
      for (var i = 0; i < idValues.length; i++) {
      ...
      if (rowIndex > 0) {
        ...
        sheet.getRange(rowIndex, 1, 1, 1).setValue(data.date);
        sheet
          .getRange(rowIndex, 5, 1, 4)
          .setValues([
            [source + " -> " + dest, finalKirim, finalChiqim, data.note],
          ]);
  ...
  data.forEach(function (row) {
    ...
    if (dateVal.getFullYear() === currentYear) {
      ...
      var isValidIncome =
        ...
        blacklist.indexOf(point.toUpperCase()) === -1;
      var isValidExpense =
        chiqim > 0 && category !== "O'tkazma" && category !== "Transfer";
  ...
  data.forEach(function (row) {
    ...
    if (category === "O'tkazma" || category === "Transfer") {
      ...
      if (balance.hasOwnProperty(payType)) {
        ...
        // Dollar chiqimlari uchun asl USD qiymatini olish
        if (
          payType === "Dollar" &&
          chiqim > 0 &&
          category !== "O'tkazma" &&
          ...
          var originalUSD = parseFloat(row[9]) || 0;

          // Agar row[9] bo'sh bo'lsa, izohdan asl USD ni ajratib olish
          if (originalUSD <= 0 && note.indexOf("Aslida:") > -1) {
            var match = note.match(/Aslida:\s*\$?([\d.,\s]+)/i);
            if (match) {
              originalUSD = parseFloat(match[1].replace(/[\s,]/g, "")) || 0;
            }
          }

          if (originalUSD > 0) {
            val = kirim - originalUSD;
          }
        }
...
function saveUser(data) {
  ...
  if (data.row == -1) {
    ...
    sheet
      ...
      .setValues([[data.login, data.password, data.role, "active", permJson]]);
  } else {
    targetRow = parseInt(data.row);

    sheet
      .getRange(targetRow, 1, 1, 3)
      .setValues([[data.login, data.password, data.role]]);
...
function getCurrencyRate(dateStr) {
  try {
    var url =
      "https://cbu.uz/ru/arkhiv-kursov-valyut/json/all/" + dateStr + "/";
    ...
    if (rate === 0) {
      ...
      var respLatest = UrlFetchApp.fetch(urlLatest, {
    ...
    if (rate === 0)
      return {
        rate: 0,
        date: "",
        error: "USD Not Found (Date & Latest failed)",
      };
